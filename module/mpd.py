...
class HiFiGANPeriodDiscriminator(torch.nn.Module):
    ...
    def forward(self, x, flg_train: bool = False):
        ...
        if self.use_san and flg_train:
            x_fun, x_dir = self.output_conv(x, flg_train=True)
            outs += [[torch.flatten(x_fun, 1, -1), torch.flatten(x_dir, 1, -1)]]
        else:
            x = self.output_conv(x)
            outs += [torch.flatten(x, 1, -1)]
...
class HiFiGANMultiPeriodDiscriminator(torch.nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        self.supports_san_training = any(getattr(disc, "supports_san_training", False) for disc in self.discriminators)
    ...
    def forward(self, x, flg_train: bool = False):
        ...
        for f in self.discriminators:
            outs += [f(x, flg_train=flg_train)]

        return outs

    def normalize_san_weights(self) -> None:
        for disc in self.discriminators:
            if getattr(disc, "use_san", False) and hasattr(disc.output_conv, "normalize_weight"):
                disc.output_conv.normalize_weight()
