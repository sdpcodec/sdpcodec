...
def _load_s3prl_upstream(feat_type: str):
    """Load s3prl upstreams without requiring a GitHub branch probe."""
    etc_dir = Path(__file__).resolve().parent.parent
    torch_home = etc_dir / "torch_hub_cache"
    local_repo_dir = torch_home / "hub" / "s3prl_s3prl_main"
    version_file = local_repo_dir / "s3prl" / "version.txt"

    os.environ.setdefault("TORCH_HOME", str(torch_home))

    try:
        from s3prl import hub as s3prl_hub

        upstream_loader = getattr(s3prl_hub, feat_type)
    ...
    if local_repo_dir.is_dir():
        try:
            if not version_file.exists():
                # Some cached snapshots miss this package data file, but s3prl imports require it.
